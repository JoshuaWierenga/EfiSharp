...
class Program
{
    ...
    class TestStaticBaseLookups
    {
        ...
        public static void Run()
        {
            ...
            if (GenAccessor<C1>.Read() != "42Hello")
                //throw new Exception();
                Console.WriteLine("TestStaticBaseLookups Test 1 Failed");

            if (GenHolder<C2>.IntField != 43 || GenHolder<C2>.StringField != "Hello`")
                //throw new Exception();
                Console.WriteLine("TestStaticBaseLookups Test 2 Failed");

            if (GenAccessor<C3>.Read() != "85World")
                //throw new Exception();
                Console.WriteLine("TestStaticBaseLookups Test 3 Failed");
        }
    }

    //TODO Add Func and Fix delegate
    /// <summary>
    ...
    //TODO Add TypeCast.RhTypeCast_IsInstanceOf and TypeCast.RhTypeCast_CheckCast
    /// <summary>
    ...
    class TestInitThisClass
    {
        ...
        public static void Run()
        {
            if (Gen2<string>.GetFromClassParam() != "Hello")
                //throw new Exception();
                Console.WriteLine("TestInitThisClass Test 1 Failed");
    ...
    class TestVirtualMethodUseTracking
    {
        ...
        public static void Run()
        {
            ...
            if (new Derived1<C1>().As(c1) != c1)
                //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 1 Failed");

            C2 c2 = new C2();
            if (new Derived1<C2>().AsToo(c2) != c2)
                //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 2 Failed");

            // Also test the stability of the vtables.
            Base2<string> b1 = new Derived2<string>();
            if (b1.Method1() != "Derived2.Method1")
                //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 3 Failed");
            Base2<object> b2 = new Derived2<object>();
            if (b2.Method2() != "Derived2.Method2")
                //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 4 Failed");
            if (TestMethod1FromSharedCode(b2) != "Derived2.Method1")
                //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 5 Failed");
            if (TestMethod1FromSharedCode(b1) != "Derived2.Method1")
                //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 6 Failed");
            if (TestMethod2FromSharedCode(b2) != "Derived2.Method2")
                    //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 8 Failed");
            if (TestMethod2FromSharedCode(b1) != "Derived2.Method2")
                //throw new Exception();
                Console.WriteLine("TestVirtualMethodUseTracking Test 9 Failed");
        }
    }
    ...
    class TestSlotsInHierarchy
    {
        ...
        public static void Run()
        {
            ...
            if (derivedAsBase.Do() != 42)
                //throw new Exception();
                Console.WriteLine("TestSlotsInHierarchy Test 1 Failed");

            if (derived.Cast("Hello") != "Hello")
                //throw new Exception();
                Console.WriteLine("TestSlotsInHierarchy Test 2 Failed");
        }
    }

    //TODO Add String.Empty, MethodInfo, Type.GetTypeInfo and Func
    ...
    //TODO Add ThreadStaticAttribute
    ...
    //TODO Add ArrayHelpers.NewObjArray, EETypePtr.IsSzArray, EETypePtr.ArrayElementType, EETypePtr.RawValue, EETypePtr.ArrayRank, Exception Support, Array.CreateInstance and Array.NewMultiDimArray
    class TestConstrainedMethodCalls
    {
        ...
        public static void Run()
        {
            ...
            if (foo1.FrobbedValue != 12345)
                //throw new Exception();
                Console.WriteLine("TestConstrainedMethodCalls Test 1 Failed");

            // Also check we passed the right generic context to Foo.Frob
            if (!result)
                //throw new Exception();
                Console.WriteLine("TestConstrainedMethodCalls Test 2 Failed");
            ...
            if (DoFrob<Foo<Atom2>, Atom2>(ref foo2, new object()))
                //throw new Exception();
                Console.WriteLine("TestConstrainedMethodCalls Test 3 Failed");
        }
    }

    //TODO Fix build errors, initially System.Runtime.CompilerServices.RawData is required but fixing that leads to linker errors with no obvious solution
    //https://i.imgur.com/CBx0cnv.png, https://i.imgur.com/N7tIi4v.png
    class TestInstantiatingUnboxingStubs
    {
        ...
        public static void Run()
        {
            ...
            if (!s_foo.IsInst("ab"))
                //throw new Exception();
                Console.WriteLine("TestInstantiatingUnboxingStubs Test 1 Failed");

            if (s_foo.IsInst(new object()))
                //throw new Exception();
                Console.WriteLine("TestInstantiatingUnboxingStubs Test 2 Failed");
            ...
            if (foo.Value != 42)
                //throw new Exception();
                Console.WriteLine("TestInstantiatingUnboxingStubs Test 3 Failed");
        }
    }

    //TODO Fix Try Catch/Exceptions
    ...
    //
    //TODO Check how this is tested, crash on build? run?
    ...
    //TODO Add Func, String.Format and Type.Name
    ...
    //TODO Type.GetElementType, Int32.ToString, Type.Name and Func
    ...
    //TODO Add Type.Name, KeyValuePair, Type.ToString, Object.ToString, Type.MakeGenericType, FieldInfo, Activator and Type.GetTypeInfo
...
#pragma warning restore
    ...
    //TODO Add Type.ToString and Fix Try Catch/Exceptions
...
#pragma warning restore 649
    ...
    //TODO Fix Delegate/MulticastDelegate
    ...
    //TODO Add Object.ToString and Type.ToString
    ...
    //TODO Fix build errors, using this function leads to linker errors with no obvious solution
    //https://i.imgur.com/N7tIi4v.png
        ...
        class GenericType<T>
        {
            ...
            public static GenericType<T> GenericMethod<U>()
            {
                ...
                if (v.NonGenericMethod() != 42)
                    //throw new Exception();
                    Console.WriteLine("TestGenericInliningDoesntHappen Test Failed");
                return v;
            }
